...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    solar_state = hass.states.get(entry.data[CONF_SOLAR_ENERGY_SENSOR])
    import_state = hass.states.get(entry.data[CONF_IMPORT_ENERGY_SENSOR])
    export_state = hass.states.get(entry.data[CONF_EXPORT_ENERGY_SENSOR])
    calculator.seed(
    ...
    def handle_grid_event(event: Event) -> None:
        import_state = hass.states.get(entry.data[CONF_IMPORT_ENERGY_SENSOR])
        export_state = hass.states.get(entry.data[CONF_EXPORT_ENERGY_SENSOR])
        price_state = hass.states.get(entry.data[CONF_EXPORT_PRICE_SENSOR])
    ...
    def handle_solar_event(event: Event) -> None:
        solar_state = hass.states.get(entry.data[CONF_SOLAR_ENERGY_SENSOR])
        price_state = hass.states.get(entry.data[CONF_IMPORT_PRICE_SENSOR])
    ...
    data.remove_listeners.extend(
        [
            async_track_state_change_event(
                hass,
                [
                    entry.data[CONF_IMPORT_ENERGY_SENSOR],
                    entry.data[CONF_EXPORT_ENERGY_SENSOR],
                ],
                handle_grid_event,
            ),
            async_track_state_change_event(
                hass,
                [entry.data[CONF_SOLAR_ENERGY_SENSOR]],
                handle_solar_event,
            ),
        ]
    )
